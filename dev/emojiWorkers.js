...
const EmojiExample = new Emoji();
self.addEventListener('message', function(event) {
    const keyword = event.data
    // 收到搜索请求后执行搜索
    const searchResult = EmojiExample.search(keyword)
    const results = []
    for (const r in EmojiExample.giveEmojiResult(searchResult)) {
        results.push(r);
    }
